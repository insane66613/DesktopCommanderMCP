...
function buildStructuredContent(
  ...
  if (toolName === 'get_config') {
    return { config: {}, entries: [], uiHints: null, text, success: !isError };
  }
  ...
  if (toolName === 'write_file' || toolName === 'edit_block' || toolName === 'list_directory') {
    return {
      fileName: null,
      filePath: null,
      name: null,
  ...
  if (toolName === 'list_sessions') {
    return { sessions: [], count: 0, text, success: !isError, ...(isError ? { error: text } : {}) };
  }
  if (toolName === 'list_processes') {
    return { processes: [], count: 0, text, success: !isError, ...(isError ? { error: text } : {}) };
  }
  if (toolName === 'get_usage_stats') {
    return { text, success: !isError, stats: null, ...(isError ? { error: text } : {}) };
  }
  if (toolName === 'get_recent_tool_calls') {
    return {
      summary: text.split('\n', 1)[0] || null,
      calls: [],
      count: 0,
  ...
  if (toolName === 'export_project_file') {
    return {
      fileName: '',
      filePath: '',
      fileType: 'unsupported',
